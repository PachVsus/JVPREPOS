...
const japaneseName = data2.names.find((name) => name.language.name === "ja-Hrkt").name;
  
  // get the type of pokemon
  let type_names = data.types.map((val, index)=>val.type.name)
  // console.log(data.types);
  let data3 = await Promise.all(type_names.map(async (val) => {
    return await getType_data(`https://pokeapi.co/api/v2/type/${val}/`);
  }));
const typeNames = data3.map((typeData) => typeData.name);
...
const displayPokemonDetails = async (pokemon) => {
  ...
  if(pokemon[2] != null && pokemon[2].length > 0){
  ...
  pokemonDetailsEl.innerHTML = `
        ...
        <img class="imgBack" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-ball.png" alt="pokeball">
        </div>
